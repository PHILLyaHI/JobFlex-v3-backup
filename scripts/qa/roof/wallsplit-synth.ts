...
// ── Случай 5 (блок 6 п.1, закон ОТКЛОНЕНИЯ по §J): слабая складка
//    1.5/12 на планарном (≤0.6) регионе НЕ режется — три линейки формы
//    отвергнуты замером (фон фантомов 0.6–2.9/12 на живой черепице
//    неотделим); возврат — по слову владельца о конкретном месте ──
{
  // наклоны 0.4 и 0.275 по диагонали (|Δ∇| = 0.125 = 1.5/12)
  const r = run((x, y) => {
    const t = (x + y) / Math.SQRT2;
    return t < 0 ? 12 + 0.4 * t : 12 + 0.275 * t;
  });
  console.log(`СЛУЧАЙ 5 — слабая складка 1.5/12 на планарном регионе: расщеплений ${r.res.splits}, регионов ${r.regionKind.length}`);
  check("слабая складка НЕ режется (закон отклонения: ниже видимости живой линейки)", r.res.splits === 0 && r.regionKind.length === 1, `splits=${r.res.splits}, регионов=${r.regionKind.length}`);
}
