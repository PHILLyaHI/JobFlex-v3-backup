...
// ── B. рваная граница без линии ──
{
  ...
  const areas = r.cells.map((c) => c.areaSqft).sort((a, b) => a - b);
  // ПО-СЕГМЕНТНОЕ СПРЯМЛЕНИЕ (приказ 2026-08-31 п.1) сменило закон: сегмент
  // без wall-вердикта — ПРЯМАЯ между законными вершинами (концы цепи не
  // двигаются). Оба конца зигзага стоят на пике x=+2 → хорда x=+2, площади
  // делятся хордой (сдвиг ≈ амплитуда 2 ft × высота 50 ft = 100 sf от
  // прежнего пиксельного равенства 1246/1254; замер 1150/1350).
  check("левая ячейка = хорде по концам", areas[0], 1150, 40);
  check("правая ячейка = хорде по концам", areas[1], 1350, 40);
}
