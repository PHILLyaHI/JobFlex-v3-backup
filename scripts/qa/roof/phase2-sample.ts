...
function toValidatorSchema(model: RoofModel, footprint: FootprintPoint[]): unknown {
  ...
  for (const f of model.faces) {
    ...
    facets.push({ id: String(f.designator || f.id), pitch: Number(f.pitch) || 0, v: ring.map(vid), ...((f as { provenance?: string }).provenance === "fill" ? { fill: true } : {}), ...((f as { provenance?: string }).provenance === "measured-dsm" ? { measured: true } : {}) });
  }
