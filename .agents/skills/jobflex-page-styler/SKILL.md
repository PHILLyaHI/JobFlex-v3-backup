...
Read root `AGENTS.md` and `DESIGN.md` first; they override conflicting reference examples. The references and dashboard asset are preserved from the Claude skill. Use their design details without assuming their old environment notes or route paths still describe the checkout. The live app uses the established ≤768px mobile switch; the 860px rules and inline-script checks below apply to standalone reference-style HTML prototypes only. Edit files using the available Codex file tools, and resolve temporary output paths explicitly instead of assuming a provider-defined scratch variable.
...
For a standalone HTML prototype, the following Node checks are a reference script. Confirm the current runtime and shell first. Replace `<file>` and `$SCRATCH` with explicit paths in a writable task directory. The snippet uses Bash quoting: on Windows, save its JavaScript body to a temporary `.cjs` file and invoke Node with separate path arguments. Do not paste Bash syntax into PowerShell. For React/Next.js edits, use the project's relevant typecheck/lint commands and browser verification instead of applying inline-script assertions to TSX.
...
- Mock: `jobflex-<page>-blueprint.html` in an explicit writable task output
  directory — one self-contained HTML file. Give a clickable absolute path
  in the reply and use the available browser/file preview tools.
- Port into the app (after the mock is approved):
  locate the actual route in the current checkout, then use `page.tsx` + a
  scoped `<page>.module.css` where consistent with that route. New standalone
  mobile pages belong under `src/app/(mobile)/` per `AGENTS.md`. Since
...
- Make follow-up edits with a focused patch and repeat relevant checks.
...
Verification: check the current development server and browser session; do
not assume an old missing-login condition still applies. Verify changed UI
in a real browser at 390×844, as required by `AGENTS.md`, and run the relevant
typecheck/lint commands. If authentication prevents access, report the exact
blocker and request a usable session without bypassing auth or seeding the
database without approval. Static checks do not establish visual correctness;
report "not visually confirmed" when a browser check could not be completed.
