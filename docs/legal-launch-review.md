...
3. **Gmail fixes merged from main.** New grants are sealed with AES-GCM before storage and disconnect attempts Google revocation before clearing the stored grant. Legacy plaintext rows remain readable; `scripts/gmail-tokens-encrypt.ts` is a separate migration, not run by this release. Confirm its completion before claiming all stored tokens are encrypted. Google revocation is best effort, so the policy retains Google-account revocation instructions.
...
- New upstream PostHog operational and activation events include organization identifiers, plan/trade facts, feature activity, and diagnostic codes independently of browser analytics choices. The privacy policy now distinguishes these from browser tracking.
- `src/app/api/integrations/gmail/callback/route.ts`: organization-bound connection, manager requirement, token storage.
- `src/actions/settings.ts`: disconnect attempts Google revocation and clears tokens and connected-account label, while preserving separately configured reply-to/settings.
