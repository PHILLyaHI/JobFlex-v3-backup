> **Status 2026-09-15 — Milestone 1 built.** `/dashboard/hvac-estimator` (page + `src/lib/hvac/*` engine + `src/actions/hvacEstimator.ts` + three Prisma tables `HvacEstimate`, `HvacCatalogItem`, `HvacSettings`). Site lookup, video walk with the filming guide, nameplate photo reading, typed confirmations with source badges, block load at the county design day, catalog selection within Manual S, duct/NEC/gas/refrigerant checks, rate-card ledger, save and convert to proposal. QA: `scripts/qa/hvac-{engine,intake,ledger,site}.check.ts` (177 checks). Not yet: Cool Calc permit-grade report, AHRI/NEEP directory import, the twenty-real-jobs comparison, county lookup needs a billed Google key on the server (the page offers a county picker when it is missing).
