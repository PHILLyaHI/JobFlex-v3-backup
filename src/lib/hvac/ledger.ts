...
function equipmentPrice(item: CatalogItem, card: HvacRateCard): { price: number; basis: LineBasis; note: string } {
  ...
  return { price: markup(cost, card.equipmentMarkupPct), basis: "estimated", note: `No shop cost on the catalog row — rate-card default per ${item.kind === "furnace" ? "10k BTU" : "ton"}${item.tier ? ` × ${item.tier} tier` : ""} + ${card.equipmentMarkupPct}%` };
}
...
function pickCompanion(catalog: CatalogItem[], kind: CatalogItem["kind"], tons: number, heatingBtuh?: number, like?: { brand?: string; tier?: CatalogItem["tier"] }): CatalogItem | undefined {
...
const KIND_WORD: Record<CatalogItem["kind"], string> = {
  "heat-pump": "heat pump", "air-conditioner": "condenser", furnace: "furnace", "air-handler": "air handler", coil: "coil", ductless: "ductless outdoor unit", package: "package unit", "water-heater": "water heater",
};
...
function systemLedger(job: JobKind, engine: EngineResult, m: BuildingModel, card: HvacRateCard, catalog: CatalogItem[], opts: LedgerOptions): Ledger {
  ...
  if (chosen) {
    ...
    mat.push(equipmentLine("eq-main", item, card, job === "ductless" ? 1 : n, sysNote));
    if (job === "ductless") {
      ...
      const wantsCoilOnly = job === "add-ac" || dualFuel || (job === "replace-outdoor" && refrigerantChanges(m, item));
      // An A2L coil on the furnace that stays: the sensor and mitigation board.
      if (wantsCoilOnly && (item.refrigerant === "R-454B" || item.refrigerant === "R-32")) mat.push({ id: "m-rds", name: "A2L refrigerant-detection sensor + mitigation board for the existing furnace", quantity: n, unitPrice: mk(card.materials.a2lSensorKit), unit: "each", basis: "estimated", note: "Required where the furnace is not listed for A2L" });
      const wantsAirHandler = (job === "replace-system" && !wantsFurnace) || allElectricHp;
      if (wantsFurnace) {
        const furnace = pickCompanion(catalog, "furnace", tons, engine.load.heatingBtuh, item);
        if (furnace) mat.push(equipmentLine("eq-furnace", furnace, card, n, sysNote));
        else {
          const input = Math.max(40000, Math.ceil((engine.load.heatingBtuh / 0.95) / 20000) * 20000);
          mat.push({ id: "eq-furnace", name: `${Math.round(input / 1000)}k BTU 95% AFUE gas furnace (matched)`, quantity: n, unitPrice: markup((input / 10000) * card.equipmentDefaults.furnacePer10kBtu, card.equipmentMarkupPct), unit: "each", basis: "estimated", note: `Sized from the ${engine.load.heatingBtuh.toLocaleString("en-US")} BTU/h heating load ÷ 0.95 AFUE — swap for a catalog row` });
        }
      }
      if (wantsFurnace || wantsCoilOnly) {
        ...
        const why = job === "replace-outdoor" ? "the new refrigerant needs a matched coil" : dualFuel ? "on the existing furnace (dual fuel)" : undefined;
        mat.push({ id: "eq-coil", name: coil ? `${coil.brand} ${coil.model} — matched evaporator coil` : `${tons}-ton matched evaporator coil`, quantity: n, unitPrice: cp.price, unit: "each", basis: cp.basis, note: [cp.note, why, sysNote].filter(Boolean).join(" · ") });
      }
      if (wantsAirHandler) {
        ...
        mat.push({ id: "eq-ah", name: ah ? `${ah.brand} ${ah.model} — matched air handler` : `${tons}-ton matched air handler`, quantity: n, unitPrice: ap.price, unit: "each", basis: ap.basis, note: [ap.note, sysNote].filter(Boolean).join(" · ") });
    ...
    if (job === "replace-furnace") {
      const input = Math.max(40000, Math.ceil((engine.load.heatingBtuh / 0.95) / 20000) * 20000);
      mat.push({ id: "eq-main", name: `${Math.round(input / 1000)}k BTU 95% AFUE gas furnace (no catalog match)`, quantity: 1, unitPrice: markup((input / 10000) * card.equipmentDefaults.furnacePer10kBtu, card.equipmentMarkupPct), unit: "each", basis: "estimated", note: "Sized from the heating load — pick a catalog row" });
    } else mat.push({ id: "eq-main", name: `${fallbackTons}-ton ${job === "ductless" ? "ductless system" : "unit"} (no catalog match — pick a unit)`, quantity: 1, unitPrice: markup(fallbackTons * perTon, card.equipmentMarkupPct), unit: "each", basis: "estimated", note: "Rate-card default per ton until a catalog row fits" });
    if (job === "ductless") mat.push({ id: "eq-heads", name: "Indoor heads (wall-mount)", quantity: heads, unitPrice: markup(card.equipmentDefaults.ductlessHeadEach, card.equipmentMarkupPct), unit: "each", basis: "estimated" });
  }
  ...
  if (job === "replace-furnace") stock("m-gasflex", "Gas flex connector, shutoff and drip leg", 1, "each", card.materials.gasFlexKit);
  if (newFurnace) {
    stock("m-vent", condensing ? "PVC vent and intake, termination and hangers" : "Vent kit (B-vent), termination and hangers", n, "each", card.materials.ventKit);
  ...
  if (job === "replace-furnace" || allElectricHp) task("l-remove-furnace", "Remove the old furnace", n, "each", L.removeFurnace);
  if (isPackage) task("l-set", "Set the package unit, curb adapter, connections", n, "each", L.setPackage, sysNote);
    ...
    else if (mat.some((l) => l.id === "eq-coil")) task("l-indoor", "Set the coil on the existing furnace", n, "each", L.setCoil, sysNote);
    if (outdoorNew) {
  ...
  if (job === "replace-furnace" || (job === "replace-system" && mat.some((l) => l.id === "eq-furnace"))) task("l-gas", "Gas: connect the furnace, shutoff, drip leg, leak test", n, "each", L.gasConnect, sysNote);
  if (check("gas") === "fix") { const ft = Math.max(10, Math.round(m.gas.longestRunFt ?? 20)); task("l-gaspipe", "Gas pipe upsizing to the furnace", ft, "ln ft", L.gasPipePerFt, `${engine.checks.find((c) => c.id === "gas")?.detail ?? ""} · ${m.gas.longestRunFt ? "run entered" : "20 ft run assumed"}`, m.gas.longestRunFt ? "entered" : "estimated"); }
  ...
  task("l-startup", "Start-up, charge, airflow and static check, homeowner walkthrough", n, "each", L.startup, sysNote);
  ...
  const what: Record<JobKind, string> = {
    ...
    ductless: `Add a ductless mini-split at ${m.address}: ${sys} with ${heads} indoor head${heads === 1 ? "" : "s"}.`,
    "water-heater": "", ducts: "", service: "",
  };
  const includes: Record<JobKind, string> = {
    "replace-system": "Includes removal and refrigerant recovery, new equipment set, line set, electrical, drain, thermostat, start-up and commissioning, permit and disposal.",
    "replace-outdoor": "Includes removal and recovery, the new unit set and connected, pressure test and evacuation, start-up, permit and disposal.",
    "replace-furnace": "Includes removal, the new furnace set, gas and vent connected, electrical reconnect, start-up, permit and disposal.",
...
export function tiersFor(engine: EngineResult, m: BuildingModel, card: HvacRateCard, catalog: CatalogItem[], opts: LedgerOptions = {}): Array<{ tier: NonNullable<CatalogItem["tier"]>; candidate: SelectionCandidate; subtotal: number }> {
  const chosen = engine.selection.chosen;
  if (!chosen) return [];
  ...
  return (["value", "mid", "premium"] as const).flatMap((tier) => {
    ...
    if (!best) return [];
    const l = buildLedger({ ...engine, selection: { ...engine.selection, chosen: best } }, m, card, catalog, opts);
    return [{ tier, candidate: best, subtotal: l.subtotal }];
  });
}

export function pickWaterHeater(catalog: CatalogItem[], plan: { fuel: string; type: string; gallons: number; vent?: string }): CatalogItem | undefined {
  const rows = catalog.filter((c) => c.kind === "water-heater" && (c.fuel ?? "gas") === plan.fuel && (c.whType ?? "tank") === plan.type);
  if (!rows.length) return undefined;
  if (plan.type === "tankless") return rows[0];
  const vented = rows.filter((c) => !c.vent || !plan.vent || c.vent === plan.vent || plan.vent === "none");
  const pool = vented.length ? vented : rows;
  return pool.filter((c) => (c.gallons ?? 0) >= plan.gallons).sort((a, b) => (a.gallons ?? 0) - (b.gallons ?? 0))[0] ?? pool.sort((a, b) => (b.gallons ?? 0) - (a.gallons ?? 0))[0];
}
...
function waterHeaterLedger(engine: EngineResult, m: BuildingModel, card: HvacRateCard, opts: LedgerOptions, catalog: CatalogItem[] = []): Ledger {
  ...
  const row = pickWaterHeater(catalog, plan);
  const eqName = row ? `${row.brand} ${row.model} — ${row.gallons ? `${row.gallons} gal ` : ""}${generic.replace(/^\d+ gal /, "").toLowerCase()}${row.uef ? ` · ${row.uef} UEF` : ""}` : generic;
  if (row) {
  ...
  } else mat.push({ id: "eq-main", name: eqName, quantity: 1, unitPrice: markup(eqCost, card.equipmentMarkupPct), unit: "each", basis: opts.input?.wh?.gallons ? "entered" : "estimated", note: `${plan.sizedFrom} · rate-card default + ${card.equipmentMarkupPct}%` });
  ...
  if (plan.fuel !== "electric" && plan.type !== "heat-pump") {
    ...
    stock("m-vent", plan.type === "tankless" || plan.vent === "power" || plan.vent === "direct" ? "Vent kit (PVC / concentric), termination" : "B-vent sections, cap and connector", card.materials.ventKit);
  ...
  if (plan.fuel !== "electric" && plan.type !== "heat-pump") { task("l-gas", "Gas: connect, shutoff, drip leg, leak test", 1, "each", L.gasConnect); task("l-vent", "Venting: run and terminate", plan.type === "tankless" || plan.vent !== "atmospheric" ? 12 : 6, "ln ft", L.ventingPerFt, plan.type === "tankless" || plan.vent !== "atmospheric" ? "12 ft of PVC assumed — measure to the termination" : "6 ft of B-vent assumed — reuse where sound"); }
  ...
  const assumptions = [...plan.notes, `Priced from the shop rate card by the task: equipment +${card.equipmentMarkupPct}%, materials +${card.materialsMarkupPct}%.`];
  const subtotal = r2([...mat, ...lab].reduce((a, l) => a + l.quantity * l.unitPrice, 0));
  return {
    title: `${eqName.replace(/^(\d+ gal )/, "")} replacement — ${m.address.split(",")[0]}`,
    scope: `Replace the water heater at ${m.address} with a ${eqName.toLowerCase()} (${plan.sizedFrom}). Includes removal and disposal, the new unit set and connected, expansion tank, T&P discharge, drain pan, ${plan.fuel !== "electric" && plan.type !== "heat-pump" ? "gas connection and venting" : "the electrical circuit"}, permit and inspection.`,
...
function ductsLedger(engine: EngineResult, m: BuildingModel, card: HvacRateCard, opts: LedgerOptions): Ledger {
  ...
  const replace = m.ducts.condition === "poor";
  const L = card.labor;
  const check = (id: string) => engine.checks.find((c) => c.id === id)?.status;
  if (replace) {
  ...
  if (check("return") === "fix") {
    mat.push({ id: "m-return", name: "Return grille and duct upsize", quantity: 1, unitPrice: mk(card.materials.returnGrilleUpsize), unit: "each", basis: "estimated", note: engine.checks.find((c) => c.id === "return")?.detail });
  ...
  if (replace) { lab.push({ id: "l-permit", name: "Mechanical permit and inspection", quantity: 1, unitPrice: card.permitFee, unit: "lot", basis: "estimated" }); lab.push({ id: "l-disposal", name: "Haul-off of the old duct", quantity: 1, unitPrice: card.disposalFee, unit: "lot", basis: "estimated" }); }
  const assumptions = [...engine.notes.filter((x) => x.kind === "assumption").map((x) => x.text), `Priced from the shop rate card by the register: materials +${card.materialsMarkupPct}%.`];
  ...
  const subtotal = r2([...mat, ...lab].reduce((a, l) => a + l.quantity * l.unitPrice, 0));
  return {
    title: `${replace ? "Duct replacement" : "Duct sealing"} — ${m.address.split(",")[0]}`,
    scope: `${replace ? "Replace the supply runs" : "Seal and re-hang the accessible ducts"} at ${m.address} (${regs} registers, ducts in the ${m.ducts.location}); ${lab.some((l) => l.id === "l-return") ? "upsize the return; " : ""}airflow and static checked after the work. Design airflow ${engine.load.coolingCfm.toLocaleString("en-US")} CFM.`,
    materials: mat, labor: lab, assumptions, subtotal,
  };
}
...
function serviceLedger(engine: EngineResult, m: BuildingModel, card: HvacRateCard, opts: LedgerOptions): Ledger {
  ...
  if (s?.refrigerantLb && s.refrigerantLb > 0) {
    mat.push({ id: "m-refr", name: `Refrigerant ${m.existing.refrigerant ?? ""}`.trim(), quantity: s.refrigerantLb, unitPrice: markup(m.existing.refrigerant === "R-22" ? card.materials.refrigerantR22PerLb : card.materials.refrigerantPerLb, card.materialsMarkupPct), unit: "lb", basis: "entered", note: m.existing.refrigerant === "R-22" ? "Reclaimed R-22 at today's cost" : undefined });
    lab.push({ id: "l-refr", name: "Leak check, recover and recharge", quantity: s.refrigerantLb, unitPrice: L.refrigerantPerLb, unit: "lb", basis: "entered" });
  }
  ...
  return {
    ...
    scope: `Service call on the ${existingWords(m)} at ${m.address}: diagnostic${s?.task ? `, ${s.task.toLowerCase()}` : ""}${s?.refrigerantLb ? `, recharge ${s.refrigerantLb} lb` : ""}${s?.parts?.length ? `, parts: ${s.parts.map((p) => p.name).filter(Boolean).join(", ")}` : ""}.`,
    materials: mat, labor: lab, assumptions, subtotal,
  };
}
