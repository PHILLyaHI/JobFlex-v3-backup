...
// ── Quota discipline (merged in from the local branch) ───────────────────────
// The allowance is ALLTIME. Running it dry turns every later lookup into a
// silent "no parcel", which downstream reads as "one building" — so spending
// stops above a reserve, loudly, and the remaining count survives restarts.

/**
 * Stop spending with this much of the ALLTIME allowance left. The reserve is
 * for the fence estimator, which is interactive and user-facing.
 */
...
/**
 * May a caller spend a parcel result right now? Without the floor a caller
 * reaches the end of an ALLTIME allowance and every later lookup fails as
 * "no parcel", which downstream reads as "one building".
 */
...
export async function lookupSiblingParcels(
  ...
  // 3 — spend, once. Not below the reserve: siblings are an enrichment, and
  // an enrichment is the first thing to give up quota.
  const wkt = sweepPolygon(subject.rings);
  if (!wkt || !isReportAllEnabled()) return [];
  if (!(await maySpendParcelQuota()).ok) return [];
  try {
...
export async function lookupParcelByPoint(
  ...
  // 3 — spend, but not below the reserve. Refusing loudly beats a silent
  // "no parcel" for the rest of the key's life; the cache above still serves.
  const gate = await maySpendParcelQuota();
  if (!gate.ok) {
    return { ok: false, reason: "error", error: gate.reason };
  }
  try {
