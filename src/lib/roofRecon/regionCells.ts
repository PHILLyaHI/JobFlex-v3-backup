...
export function buildRegionCells(input: RegionCellsInput): RegionCellsResult {
  ...
  for (const sp of simped) {
    for (let i = 0; i + 1 < sp.pts.length; i++) {
      ...
      // Прова — от ИСТОЧНИКА метки: манхэттен-вершина (m) выправлена к
      // осям кольца, не к несущей — её ребро не наследует lineIndex
      // (стена-манхэттен: срединная проекция тащила узел на 3.9 ft на
      // вальма-фикцию, кольцо A2 заступало за стену, уклон 8.0 → 5.6).
      // Гибриды (m только на wallPair, пороги шаг/2·шаг/σ⊥) отвергнуты
      // замером — качели G4/R03 по 12618/12629; источник-закон держит
      // 12629+12618. Остаток 12621 (G1 53.9° на cm-вершине честной
      // складки у рваной кромки) — в стоп-отчёте.
      const straight = sp.li !== undefined
        && (sp.pts[i].c ?? false) && (sp.pts[i + 1].c ?? false)
        && !(sp.pts[i] as { m?: boolean }).m && !(sp.pts[i + 1] as { m?: boolean }).m;
      if (sp.interCluster) {
