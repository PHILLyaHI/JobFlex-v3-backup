// Server-only: push a PricingPlan's monthly (+ optional yearly) price to Stripe.
//
// Extracted from the syncPlanToStripe server action so upsertPricingPlan can
// auto-sync after a price change. Deliberately NOT a "use server" export (those
// become public RPC endpoints) and carries NO auth check of its own — callers
// (admin actions) must gate with requirePlatformAdmin. Revalidation is also the
// caller's job.
//
...
// Existing subscribers are grandfathered: archiving a Stripe Price only blocks
// NEW checkouts from using it; live subscriptions keep billing at it.
...
export async function syncPlanPricesToStripe(planId: string): Promise<{ ok: true }> {
  if (!isStripeEnabled()) {
  ...
  for (const spec of intervals) {
    ...
    // Skip when the active mirror already carries this exact amount — keeps the
    // auto-sync idempotent (no duplicate Stripe Prices on no-op saves).
    const current = await db.planPrice.findFirst({
      where: { planSlug: plan.slug, interval: spec.interval, active: true },
    });
    if (current && current.unitAmountCents === spec.amount && current.stripeProductId === productId) {
      continue;
    }
