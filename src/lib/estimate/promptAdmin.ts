...
import { resolveOpenAIModel } from "@/lib/sdk/openai";
 ...
 *  The model is RESOLVED, never the env name: a key whose project is not
 *  entitled to it runs on the default, and the preview has to show what the
 *  estimate will really do (lib/sdk/openai.resolveOpenAIModel). */
export const isReasoningModel = (model: string) => /^(gpt-5|o[1-9])/.test(model);
