    ...
    if (pricingPrompt?.trim()) {
      ...
      lines.push(
        [
          ...
          "Return one pricing lineItem per step of the PROCEDURE block above — every core step, in its order, plus each conditional step the brief calls for — priced at the PRICE BOOK's step costs scaled by the LOCATION factor. Never fold several steps into one lump line, and never fit the lines to a total you picked first.",
          ...
          "All totals should equal the appropriate calculation (sqft × unitPrice for 'sqft', quantity × unitPrice for 'linear'/'linearft'/'sqboards'/'cubic'/'unit'/'hour', fixedPrice for 'fixed'), and recommendedPrice is the sum of the line item totals, computed last from the lines.",
