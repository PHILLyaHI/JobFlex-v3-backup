...
export function validateRoofInvariants(model: RoofModel, opts: InvariantOptions = {}): InvariantReport {
  ...
  const facets = model.faces
    .map((f, i) => {
      ...
      return {
        ...
        // R03-бюджет по провенансу: измеренная грань несёт шум конвейера
        // (вершины на пересечениях пар/кластерных плоскостях: популяция
        // невязок бимодальна — шум ≤0.3, стены ≥1.8; порог в зазоре, как
        // переписной пол). Скелетные/фикстурные грани — прежний бюджет.
        meps: (f as { provenance?: string }).provenance === "measured-dsm" ? 0.3 : INV_EPS_PLANE,
      };
    })
  ...
  const vertFacets = new Map<string, typeof facets>();
  const vKey3 = (p: IPt3): string => invKey([p[0], p[1]]) + "|" + Math.round(p[2] * 1000);
  for (const f of facets) for (const p of f.pts3) {
    const arr = vertFacets.get(vKey3(p)) ?? ([] as typeof facets);
    if (!arr.includes(f)) arr.push(f);
    vertFacets.set(vKey3(p), arr);
  }
  const devTo = (pl: InvPlane, p: IPt3): number => Math.abs(pl.a * p[0] + pl.b * p[1] + pl.c - p[2]);
  ...
  const weldAllowance = (f2: (typeof facets)[number]): { ok: boolean; dev: number; excused: number } => {
    ...
    for (const p of f2.pts3) {
      ...
      for (const g of vertFacets.get(vKey3(p)) ?? []) {
        ...
        const slack = Math.max(plF.maxDev, f2.meps) + Math.max(plG.maxDev, INV_EPS_PLANE);
        if (p[2] >= Math.min(zF, zG) - slack && p[2] <= Math.max(zF, zG) + slack) { pardon = true; break; }
      }
      if (!pardon) {
        // ВЕРШИНА БЕЗ СВИДЕТЕЛЕЙ ПРОЛЁТА: одиночка (план-двойник уровня
        // совладельцем не является) ИЛИ все совладельцы отстоят на стену
        // (клин: |zF−zG| ≥ пол переписи − сварные бюджеты — их плоскость
        // о месте вершины НЕ свидетельствует, вершина живёт на ВЕРХНЕЙ).
        // Референс — своя чистая плоскость, слак — её фактический остаток
        // (тот же закон, что слак пары): судить точнее остатка линейки —
        // судить шум, не геометрию.
        ...
        const noSpanWitness = others2.every((g2) => {
          ...
          return Math.abs(zF2 - zG2) >= 1.8 - 4 * INV_EPS_PLANE;
        });
        // чистый фит откалиброван БЕЗ подсудимой вершины — судить её
        // точнее ПОЛНОГО остатка кольца линейка не вправе
        if (process.env.DBG_R03) console.log(`[R03·nsw] ${f2.id}: dF=${dF.toFixed(3)} plF.maxDev=${plF.maxDev.toFixed(3)} full=${(f2.plane?.maxDev ?? 0).toFixed(3)} nsw=${noSpanWitness}`);
        if (noSpanWitness && dF <= Math.max(plF.maxDev, f2.plane?.maxDev ?? 0, f2.meps) + INV_EPS_PLANE) pardon = true;
      }
      if (!pardon) {
        if (process.env.DBG_R03) {
          for (const g of vertFacets.get(vKey3(p)) ?? []) {
            ...
            const zG2 = plG ? plG.a * p[0] + plG.b * p[1] + plG.c : NaN;
            const plF2 = cleanPlane.get(f2.id);
            const zF2 = plF2 ? plF2.a * p[0] + plF2.b * p[1] + plF2.c : NaN;
            // eslint-disable-next-line no-console
            console.log(`   [R03·span] ${f2.id}|${g.id}: v.z=${p[2].toFixed(2)} zF=${zF2.toFixed(2)} zG=${zG2.toFixed(2)} чистG=${plG ? plG.maxDev.toFixed(2) : "нет"}`);
          }
  ...
  for (const f of facets) {
    ...
    // сварное смещение когерентно вращает полное кольцо (R03 v2 / G4).
    // Лента уже разрешающей ширины измерения (model.resolutionFt — полное
    // окно нормалей) собственного уклона не несёт: подгонка её кольца
    // нестабильна by nature (полосы скругления гребня 24x1.2 ft)
    ...
    const mFull = invGrad(f.plane) * 12;
    const mClean = cleanPlane.get(f.id) ? invGrad(cleanPlane.get(f.id)!) * 12 : mFull;
  ...
  // Граф грамматики строится из ЗАЯВЛЕННЫХ ЛИНИЙ напрямую, а не из колец
  // граней: кольцо с R02-щипком не сшивается, полмира (швы, соседние
  // складки) исчезает из графа, и легальная терминация на шве читалась
  // «обрывом в поле» (§K: линейка, искажающая измеряемое, — вторично).
  // Без model.lines (чистые фикстуры скилла) — фолбэк на рёбра колец.
  interface GEdge2 { a: IPt3; b: IPt3; t: string; d: string; facets: Array<{ id: string }> }
  const byTypeEdges = [...byType.eave, ...byType.rake, ...byType.ridge, ...byType.hip, ...byType.valley, ...byType.unknown];
  const gAllEdges0: GEdge2[] = [];
  if (model.lines?.length && model.points?.length) {
    const pById = new Map(model.points.map((pt) => [pt.id, pt]));
    const ownersOfL = new Map<string, Array<{ id: string }>>();
    for (const f of model.faces) for (const id of new Set(f.lineIds)) {
      const arr = ownersOfL.get(id) ?? [];
      arr.push({ id: f.id });
      ownersOfL.set(id, arr);
    }
    for (const l of model.lines) {
      ...
      const t =
        l.type === "RIDGE" ? "ridge" :
        l.type === "HIP" ? "hip" :
        l.type === "VALLEY" ? "valley" :
        l.type === "EAVE" ? "eave" :
        l.type === "RAKE" ? "rake" :
        l.type === "FLASHING" || l.type === "STEPFLASH" ? "seam" : "unknown";
      gAllEdges0.push({ a: [a.x, a.y, a.z], b: [b.x, b.y, b.z], t, d: l.type, facets: ownersOfL.get(l.id) ?? [] });
    }
  } else {
    for (const e of byTypeEdges) gAllEdges0.push({ a: e.a, b: e.b, t: e.type ?? "unknown", d: "", facets: e.facets });
  }
  const gTypeOf = (e: GEdge2): string => e.t;
  ...
  // терминироваться.
  // ШОВ — ГЕОМЕТРИЯ, не тип (класс 1a смотра RM-6KT8LW): пара рёбер одной
  // план-позиции с разным z (по концам, максимумом — закон клин-стены) —
  // стена/край секции, каким бы типом стороны ни назвались: край массы
  // над землёй носит EAVE/RAKE нижнего уровня, не FLASHING.
  const gPlanKeyE = (e: GEdge2): string => {
    const k1 = invKey([e.a[0], e.a[1]]);
    const k2 = invKey([e.b[0], e.b[1]]);
    return k1 < k2 ? `${k1}#${k2}` : `${k2}#${k1}`;
  };
  const gByPlanE = new Map<string, GEdge2[]>();
  ...
  const gSeamGeom = (e: GEdge2): boolean => {
    const key1 = invKey([e.a[0], e.a[1]]);
    for (const o of gByPlanE.get(gPlanKeyE(e)) ?? []) {
      if (o === e) continue;
      const same = invKey([o.a[0], o.a[1]]) === key1;
      const oa = same ? o.a : o.b;
      const ob = same ? o.b : o.a;
      if (Math.max(Math.abs(oa[2] - e.a[2]), Math.abs(ob[2] - e.b[2])) >= 1.8) return true; // G_STEP_DZ
    }
    return false;
  };
  const gSeams: GEdge2[] = gAllEdges0.filter((e) => gTypeOf(e) === "seam" || gSeamGeom(e));
  const gIsSeam = new Set(gSeams);
  const gDeg = new Map<string, GEdge2[]>();
  ...
  const gOn = (p: IPt3, list: GEdge2[]): boolean => list.some((r) => gDistSeg(p, r.a, r.b) <= STUB_FT);
  ...
  for (const [k, arr] of gDeg) {
    ...
    const other = (e: GEdge2): IPt3 => (invKey([e.a[0], e.a[1]]) === k ? e.b : e.a);
    ...
    // излом материален, когда вершина отходит от хорды дальше бюджета
    // планарности (0.08) — та же величина, что в слиянии звеньев
    const chord = Math.hypot(B[0] - A[0], B[1] - A[1]) || 1;
    const perpP = Math.abs((P[0] - A[0]) * (B[1] - A[1]) - (P[1] - A[1]) * (B[0] - A[0])) / chord;
    if (perpP > INV_EPS_PLANE)
  ...
  for (const e of gCreases) {
    ...
    for (const p of [e.a, e.b]) {
      ...
      if (t === "valley") {
        ...
        // конёк с зазором до настоящего узла прятался за таким (12629 п.4)
      ...
      if (!okEnd && process.env.DBG_G) {
        ...
        err("G2", `${t}: терминация вне грамматики в (${p[0].toFixed(1)},${p[1].toFixed(1)}) — ${why} [deg=${arr.length}; рядом: ${near.map((o) => o.t + (gIsSeam.has(o) ? "*" : "")).join(",")}; швов=${gSeams.length}]`);
  ...
  const gGrad = new Map<string, [number, number]>();
  ...
  {
    const basePl = (f2: (typeof facets)[number]): InvPlane | null => cleanPlane.get(f2.id) ?? f2.plane;
    ...
    for (const [k3, fs3] of vertFacets) {
      ...
      if (zs3.length >= 2 && Math.max(...zs3) - Math.min(...zs3) > INV_EPS_PLANE) disputed.add(k3);
    }
    for (const f of facets) {
      // лента уже разрешающей ширины не несёт градиента (симметрично R04):
      // вырожденная подгонка полосы гребня давала «уклон» 22/12 и валила
      // G4 соседнего конька на 19.7°
      if (model.resolutionFt) {
        let perR = 0;
        for (let iR = 0; iR < f.plan.length; iR++) {
          const q1 = f.plan[iR];
  ...
  const gWidth = new Map<string, number>();
  for (const f of facets) {
    let perW = 0;
    ...
    gWidth.set(model.faces[f.i].id, (2 * Math.abs(f.planArea)) / Math.max(perW, 1e-9));
  }
  for (const e of gAllEdges0.filter((o) => o.t === "ridge")) {
    ...
    for (const f of e.facets) {
      ...
      const tolF = wF > 0 ? (Math.atan((2 * INV_EPS_PLANE) / (gm * wF)) * 180) / Math.PI : 0;
  ...
  const gMid = (e: GEdge2): IPt3 => [(e.a[0] + e.b[0]) / 2, (e.a[1] + e.b[1]) / 2, 0];
  ...
  const g8FaceAt = (x: number, y: number) => {
    for (const f of facets) if (invPointInPoly([x, y], f.plan)) return f;
    return null;
  };
  const g8SideCheck = (e: GEdge2) => {
    ...
    for (const s of [1, -1]) {
      ...
      if (!invPointInPoly([px2, py2], fp)) continue; // за контуром — ободок наружу
      // при перекрытии колец сторону судит ЛУЧШАЯ из накрывающих граней
      // (минимальный |d| — та, что реально встречает линию)
      let best = null as null | { d: number; slack: number };
      for (const f of facets) {
        if (!invPointInPoly([px2, py2], f.plan)) continue;
        const pl = cleanPlane.get(f.id) ?? f.plane;
        if (!pl) continue;
        const d2 = pl.a * mx + pl.b * my + pl.c - mz;
        const slack2 = 2 * INV_EPS_PLANE + Math.max(pl.maxDev - INV_EPS_PLANE, 0);
  ...
  };
  {
    ...
    const g8Edges = new Map<string, Array<{ f: string; a: IPt3; b: IPt3 }>>();
    for (const f of facets) {
      for (let i8 = 0; i8 < f.pts3.length; i8++) {
        const A8 = f.pts3[i8];
        const B8 = f.pts3[(i8 + 1) % f.pts3.length];
        const ka8 = invKey([A8[0], A8[1]]);
        const kb8 = invKey([B8[0], B8[1]]);
        if (ka8 === kb8) continue;
    ...
    for (const arr8 of g8Edges.values()) {
      ...
      if (arr8.length >= 2) {
        ...
        const wedge8 = (dzA8 >= G_STEP_DZ - 4 * INV_EPS_PLANE && dzB8 <= 2 * INV_EPS_PLANE) || (dzB8 >= G_STEP_DZ - 4 * INV_EPS_PLANE && dzA8 <= 2 * INV_EPS_PLANE);
        if (!wedge8 && dz8 > 2 * INV_EPS_PLANE && dz8 < G_STEP_DZ - 4 * INV_EPS_PLANE) {
          g8bad++;
        ...
        if (!invPointInPoly([m8[0], m8[1]], fp)) continue; // чужая структура — не об этот контур
        ...
        if (g8SideCheck(eTmp as never).below === 0) {
          g8bad++;
  ...
  for (const e of gAllEdges0) {
    ...
    const m5 = gMid(e);
    if (!invPointInPoly([m5[0], m5[1]], fp)) continue;
    const worst = Math.max(gRingDist(e.a), gRingDist(e.b), gRingDist(m5));
    if (worst <= STUB_FT) continue;
    // законные исключения: верх НАСТОЯЩЕЙ ступени (близнец с Δz от
    // переписного пола) ИЛИ ободок МАССЫ (по ту сторону — ГРАНЬ крыла
    // ниже на ≥ пол; форма обязательна — без неё исключение было дырой
    // для фантомов)
    if (gTwinDz(e) >= G_STEP_DZ - 4 * INV_EPS_PLANE) continue;
  ...
  for (const e of gAllEdges0) {
    ...
    if (!onRing) continue;
    const dz = gTwinDz(e);
    if (dz < G_STEP_DZ - 4 * INV_EPS_PLANE)
  ...
  for (const e of gAllEdges0) {
    for (const p of [e.a, e.b]) {
      ...
      if (!invPointInPoly([p[0], p[1]], fp)) continue; // чужая структура — не об этот контур
      let best = Infinity;
