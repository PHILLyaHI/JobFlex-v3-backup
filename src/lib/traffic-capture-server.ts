...
/** `variant` (landing-e pass A): "e" for the test landing's signups, absent otherwise — the funnel reads absent as d. */
export async function captureSignupOutcome(identity: unknown, sessionId: string, outcome: string, plan: string | null, live: boolean, industry: string | null = null, utm: Record<string, string | undefined> | null = null, variant: string | null = null) {
    ...
    const response = await fetch(`${host}/capture/`, {
      ...
      body: JSON.stringify({ api_key: token, event: "jf_signup_completed", distinct_id: context.distinctId,
        ...
        properties: { $insert_id: createHash("sha256").update(`jf_signup_completed:${sessionId}`).digest("hex"),
          ...
          jf_hostname: context.hostname, jf_environment: context.environment, verified: true, billing_mode: live ? "live" : "test", outcome, plan: plan || "unknown", industry: industry || "default", ...(variant ? { variant } : {}), ...utmProps } }),
