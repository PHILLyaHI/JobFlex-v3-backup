...
import { EventChip, type CalendarEvent, APPOINTMENT_BORDER } from "./EventChip";
...
export function MonthGrid({
  ...
  const eventsByDay = React.useMemo(() => {
    ...
    for (const e of events) {
      if (e.kind === "blocked") continue; // blocked time renders as a full-day cell wash
  ...
  // Blocked time, split per covered day: a day the block covers ENTIRELY gets
  // the full-cell wash; a day where only certain hours are blocked keeps the
  // regular chip (striped rose via EventChip's blocked treatment) so the cell
  // doesn't read as a whole day off.
  const blockedByDay = React.useMemo(() => {
    const m = new Map<string, { event: CalendarEvent; fullDay: boolean }[]>();
    for (const e of events) {
      if (e.kind !== "blocked") continue;
      const evStart = new Date(e.startsAt);
      const evEnd = new Date(e.endsAt);
      const first = startOfDay(evStart);
      const last = startOfDay(evEnd);
      for (let d = new Date(first); d <= last; d.setDate(d.getDate() + 1)) {
        const dayStart = new Date(d);
        const dayEnd = new Date(d);
        dayEnd.setDate(dayEnd.getDate() + 1);
        // 1-minute tolerance so a 12:00 AM – 11:59 PM block still counts as
        // covering the whole day.
        const fullDay =
          evStart.getTime() <= dayStart.getTime() &&
          evEnd.getTime() >= dayEnd.getTime() - 60_000;
        const key = isoKey(d);
        if (!m.has(key)) m.set(key, []);
        m.get(key)!.push({ event: e, fullDay });
      }
    }
    return m;
  }, [events]);
  ...
  const { spanByDay, lanesByWeek } = React.useMemo(() => {
    const multi = events.filter((e) => isMultiDay(e) && e.kind !== "blocked");
    ...
    <div className="paper-card p-0 overflow-hidden">
      ...
      <div className="grid grid-cols-7 auto-rows-fr" style={{ minHeight: 624 }}>
        {days.map((d, i) => {
          ...
          const blockedHere = blockedByDay.get(iso) ?? [];
          const blocked = blockedHere.filter((b) => b.fullDay).map((b) => b.event);
          // Partial-hour blocks render as chips ahead of the day's other events.
          const list = [
            ...blockedHere.filter((b) => !b.fullDay).map((b) => b.event),
            ...(eventsByDay.get(iso) ?? []),
          ];
          ...
          return (
            <div
              ...
              onPointerDown={() => {
                // A dragged chip that remounts mid-gesture (reschedule →
                // router.refresh) drops its framer onDragEnd, so draggingRef can
                // stick `true` and silently kill every later day-cell click
                // (create). A fresh press clears the stale flag before the click
                // guard runs; lastDragEndAt still suppresses the real trailing click.
                draggingRef.current = false;
              }}
              onClick={() => {
              ...
              className={cn(
                ...
                isToday && "bg-[color-mix(in_srgb,var(--accent-soft)_50%,transparent)]",
                previewIso === iso &&
            ...
            >
              {/* Blocked time — a full-day wash of rose diagonal stripes under
                  the day's content, with the reason pinned to the cell foot.
                  Clicking it opens the block's detail sheet. */}
              {blocked.length > 0 && (
                <button
                  type="button"
                ...
                >
                  <span className="absolute bottom-1.5 left-2 right-2 truncate text-[10px] font-medium text-[color:var(--rose)]">
                    {blocked[0].title}
              ...
              <div className="relative z-[1] flex items-center justify-between mb-1.5 pointer-events-none">
                <span
                  className={cn(
              ...
              {spans.map((b) => (
                <button
                  ...
                  style={{
                    ...
                    height: BAR_H,
                    // Blocked events never reach the multi set (they render as
                    // the full-day wash). Appointments get the blue tint; jobs
                    // are status-colored.
                    background:
                      b.event.kind === "appointment"
                        ? APPOINTMENT_BORDER
                        : statusAccent(b.event.status),
                  }}
                >
              ...
              <div className="relative z-[1] flex flex-col gap-1" style={{ marginTop: barReserve }}>
