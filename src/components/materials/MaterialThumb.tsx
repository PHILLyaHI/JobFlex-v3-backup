...
import { usableImageUrl } from "@/lib/merchantLinks";
 ...
 * missing, implausible (AI-fabricated storefront "image" links that are
 * guaranteed to 404 — see usableImageUrl), OR fails to load.
 */
export function MaterialThumb({ src, alt }: { src: string | null; alt: string }) {
  const [failed, setFailed] = React.useState(false);
  const usable = usableImageUrl(src);
  const show = Boolean(usable) && !failed;
