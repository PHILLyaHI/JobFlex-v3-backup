...
// PROOF CARD — mount contract (route: /dashboard/manual-proof).
//
// A local copy of the fleet's reveal cascade rather than an import from another
// lab folder: each variant in this lab is self-contained inside its own
...
// exactly as every blueprint page does.
//
...
// root — so the cascade works even though this page is deliberately absent from
// the shell's PAGE_STYLES map (see the scoping note in manual-proof.module.css).
//
// Every block the content component returns is a DIRECT child of `.content`,
// which is what this walks. The sticky derivation bar is one of them: `.rv`
// parks it at translateY(14px) for its slot and then sets `transform: none`,
// and a sticky box with no transform sticks normally.
...
export function useReveal() {
  ...
  const init = useCallback((content: HTMLElement) => {
    ...
    const timers = blocks.map((b, i) =>
      window.setTimeout(() => b.classList.add("rv-in"), 60 + i * 60),
    );
