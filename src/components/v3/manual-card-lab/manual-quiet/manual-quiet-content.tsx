...
// QUIET — the column. Route: /dashboard/manual-quiet.
//
...
//     changes surface colour entirely, so the scroll has beats instead of ten
//     identical slabs.
...
import { Btn, Card, Field, Group, Pair, TextArea, TextField, cx } from "./quiet-ui";
import { ClientField, ProjectField } from "./quiet-pickers";
import { LineItems } from "./quiet-lines";
import { MarkupBlock, PaymentBlock } from "./quiet-money";
import { PrintOptions, FilesBlock } from "./quiet-blocks";
import { TheirCopy } from "./quiet-proof";
 ...
 *  read. Kept as a single string on purpose — see the note in quiet-pickers. */
...
export function ManualQuietContent() {
  ...
  const reset = () => {
    ...
    setContact(contactOf(SEED_CLIENTS, makeSeedDraft().client));
    setOpenLines([]);
    setNote("clean");
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.stack}>
        ...
        {/* 08 ------------------------------------------------------- */}
        <Card num="08" title="Payment schedule" id="q-08">
          <PaymentBlock
      ...
      <div className={styles.bar}>
        ...
        {confirmReset ? (
          <div className={styles.barActions}>
            <span className={styles.barConfirm}>Discard every edit?</span>
          ...
          <div className={styles.barActions}>
            <Btn tone="quiet" onClick={() => setConfirmReset(true)}>
              Reset
            </Btn>
