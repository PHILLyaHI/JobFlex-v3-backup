// Mobile workers — mobile-workers-v2. A handheld-first rebuild of the Workers
...
// Lives beside the desktop /dashboard/workers rather than replacing it, per the
...
// the literal path — the V3_PORTED_ROUTES key is added by the route registration
// pass, not here.

import type { Metadata, Viewport } from "next";
import { redirect } from "next/navigation";
import { auth } from "@/lib/auth";
import { MobileWorkers } from "./mobile-workers";

export const dynamic = "force-dynamic";

export const metadata: Metadata = {
  title: "Workers · JobFlex Mobile",
  description: "Blueprint-edition mobile roster: who is on the books, who is on a job, and each worker's portal link.",
};
...
export default async function MobileWorkersV2Page() {
  ...
  if (!session?.user?.id) {
    redirect(`/auth/login?next=${encodeURIComponent("/mobile-workers-v2")}`);
  }
