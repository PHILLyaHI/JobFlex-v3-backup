...
// ── Field-worker "work order" · Field Command (bento) ────────────────────────
// Read-only, scoped to the worker's own assignment in the query (org +
// assignment) so a worker can never open a job they're not on. Full-width
// dashboard grid: a charcoal command hero across the top, then mixed-size tiles
// (When / Where / Status, Scope + Schedule, Crew + Photos) built to read at
// arm's length on a jobsite.
export async function WorkerJobView({
  ...
  const myStatus = job.assignments.find((a) => a.workerId === wp.id)?.status ?? "PENDING";
  const cfg = ASSIGN[myStatus] ?? ASSIGN.PENDING;
  const StatusIcon = cfg.icon;
  const address = job.client?.address ?? null;
        ...
        <header className="overflow-hidden rounded-[var(--r-lg)] bg-[color:var(--ink)] p-6 shadow-[var(--shadow-md)] sm:p-8 lg:col-span-3">
          ...
          <div className="mt-4 flex flex-wrap items-end justify-between gap-x-8 gap-y-4">
            <div className="min-w-0">
              ...
              {job.client?.name && (
                <p className="mt-2 text-[13.5px] text-white/55">for {job.client.name}</p>
              )}
            </div>
            {job.startsAt && (
        ...
        {job.assignments.length > 0 && (
          <div className="paper-card p-5">
            <TileLabel icon={Users}>
              Crew{" "}
              <span className="tabular text-[color:var(--ink-faint)]">
                {job.assignments.length}
              </span>
            </TileLabel>
            <ul className="mt-3 space-y-2.5">
              {job.assignments.map((a) => {
                const me = a.workerId === wp.id;
                return (
                  <li key={a.id} className="flex items-center gap-2.5">
                    <Avatar name={a.worker.displayName} size={28} />
                    <div className="min-w-0 text-[13px] font-semibold text-[color:var(--ink)]">
                      {a.worker.displayName}
                      {me && (
...
// ── assignment banner styling per status ──
const ASSIGN: Record<
  string,
...
> = {
  ACCEPTED: {
    ...
    sub: "You're on the crew for this job. Everything you need is here.",
  ...
  PENDING: {
    ...
    sub: "Accept or decline from your invite so the office knows you're set.",
  ...
  DECLINED: {
    ...
    sub: "Message your manager if that wasn't intentional.",
  ...
  COMPLETED: {
    icon: CheckCircle2,
    label: "Job complete",
    sub: "Nice work, this one's wrapped up.",
    wrap: "bg-emerald-50 text-emerald-900",
    iconWrap: "bg-emerald-600 text-white",
  },
};

// Future-aware day label. relative() in lib/format is past-only (future dates
// wrongly read "just now"), so field schedules get their own forward-looking copy.
