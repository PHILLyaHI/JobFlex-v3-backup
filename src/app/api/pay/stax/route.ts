...
// Public (portal) — mints a Stax invoice on the CONTRACTOR's merchant account
// for one stage or the remaining balance and hands back its hosted bill URL.
// Amount is never read from the body.
export async function POST(req: Request) {
  ...
  const res = await createCheckout({ provider: "STAX", publicId: body.publicId, target: body.target });
