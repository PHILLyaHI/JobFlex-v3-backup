...
import { updateGmailSettings } from "@/actions/settings";
...
export function GmailForm({ initial }: { initial: GmailSettings }) {
  ...
  // OAuth handshake is out of scope for this pass — flip the local connection
  // flag so the preference is editable; Save persists it.
  function connect() {
    setConnecting(true);
    setTimeout(() => {
      setConnected(true);
      setConnecting(false);
      toast.info("Connection flagged", "Gmail OAuth isn't wired yet — Save to persist this preference.");
    }, 400);
  }
          ...
          <div className="flex items-start gap-4 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <div className="text-[12px] text-[color:var(--ink-muted)] mt-1 leading-relaxed max-w-md">
                {connected
                  ? "Outbound mail routes through your account once OAuth is wired."
            ...
            {connected ? (
              <>
                ...
                <Button
                  ...
                  onClick={() => {
                    setConnected(false);
                    toast.info("Disconnected", "Save to persist this change.");
                  }}
                >
                  Disconnect
                </Button>
