...
import { Webhook, Facebook, Copy, ExternalLink, Activity, Save } from "lucide-react";
...
import { updateMetaSettings } from "@/actions/settings";
import type { MetaSettings } from "@/lib/settings";

export function MetaForm({ initial }: { initial: MetaSettings }) {
  const router = useRouter();
  const [connected, setConnected] = React.useState(initial.connected);
  const [autoCreate, setAutoCreate] = React.useState(initial.autoCreate);
  const [autoText, setAutoText] = React.useState(initial.autoText);
  const [defaultPage, setDefaultPage] = React.useState(initial.defaultPage);
  const [formCategory, setFormCategory] = React.useState(initial.formCategory);
  ...
  async function save() {
    setBusy(true);
    try {
      await updateMetaSettings({ connected, autoCreate, autoText, defaultPage, formCategory });
      toast.success("Saved", "Meta settings updated.");
      router.refresh();
    } catch (err: unknown) {
  ...
  return (
    <>
        ...
        <div className="relative flex items-start justify-between gap-6">
          <div className="flex items-start gap-4 min-w-0">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <div className="font-display text-[20px] tracking-[-0.01em]">{defaultPage}</div>
          ...
          <div className="shrink-0 flex gap-2">
            ...
            <Button
              ...
              onClick={() => {
                setConnected((x) => !x);
                toast.info(connected ? "Unsubscribed" : "Subscribed", "Save to persist this change.");
              }}
            >
      ...
      <Card className="mt-5">
        ...
        <div className="py-8 text-center text-[12px] text-[color:var(--ink-muted)] leading-relaxed">
          No deliveries yet. Once Meta is connected and sending{" "}
          <span className="font-mono">leadgen</span> webhooks, the most recent
