...
import { updateLeadsSettings } from "@/actions/settings";
import type { LeadsSettings, LeadStrategy } from "@/lib/settings";

export function LeadsSettingsForm({ initial }: { initial: LeadsSettings }) {
  const router = useRouter();
  const [strategy, setStrategy] = React.useState<LeadStrategy>(initial.strategy);
  const [aiCategorize, setAiCategorize] = React.useState(initial.aiCategorize);
  const [instantSms, setInstantSms] = React.useState(initial.instantSms);
  const [businessHours, setBusinessHours] = React.useState(initial.businessHours);
  const [autoDeclineDupes, setAutoDeclineDupes] = React.useState(initial.autoDeclineDupes);
  const [zips, setZips] = React.useState(initial.zips);
  const [travelRadius, setTravelRadius] = React.useState(initial.travelRadius);
  const [outsidePolicy, setOutsidePolicy] = React.useState(initial.outsidePolicy);
  const [maxPerDay, setMaxPerDay] = React.useState(initial.maxPerDay);
  const [maxPerRep, setMaxPerRep] = React.useState(initial.maxPerRep);
  const [smartPrompt, setSmartPrompt] = React.useState(initial.smartPrompt);
  ...
  async function save() {
    ...
    try {
      ...
      toast.success("Saved", "Lead settings updated.");
      router.refresh();
    } catch (err: unknown) {
    ...
    <>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Select
                ...
                value={outsidePolicy}
                onChange={(e) => setOutsidePolicy(e.target.value)}
              >
      ...
      <Card className="mt-5">
        ...
        <div className="divide-y divide-[color:var(--ink-line)]">
          ...
          />
          <Toggle
            checked={autoDeclineDupes}
            onChange={setAutoDeclineDupes}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mt-5">
        <Card>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            />
            <Input
              label="Max active per rep"
              type="number"
              value={maxPerRep}
              onChange={(e) => setMaxPerRep(Number(e.target.value))}
            />
          </div>
        </Card>
        ...
        <Card>
          ...
          <Select value={smartPrompt} onChange={(e) => setSmartPrompt(e.target.value)}>
          ...
          <p className="mt-3 text-[11px] text-[color:var(--ink-muted)] leading-relaxed">
            {`"Contractor-first" prompts the model to surface scope, materials, and urgency in the opening summary you see on each lead.`}
          </p>
        </Card>
      </div>
